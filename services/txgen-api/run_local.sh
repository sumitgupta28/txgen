#!/usr/bin/env bash
# Run txgen-api locally with uvicorn hot-reload.
# Kafka and MongoDB must be reachable on localhost (or override the env vars below).
#
# Usage:
#   ./run_local.sh              # start the service (port 8002, hot-reload on)
#   ./run_local.sh --setup      # only create venv + install deps, then exit
#   PORT=9002 ./run_local.sh    # run on a different port
...
log()  { echo -e "${GREEN}[txgen-api]${NC} $*"; }
warn() { echo -e "${YELLOW}[txgen-api]${NC} $*"; }
err()  { echo -e "${RED}[txgen-api]${NC} $*" >&2; }
...
cd "$SERVICE_DIR"
exec uvicorn src.main:app --host 0.0.0.0 --port "$PORT" --reload --log-level "$(echo "$LOG_LEVEL" | tr '[:upper:]' '[:lower:]')"
