#!/usr/bin/env bash
# Run rag-processor locally.
# Kafka and TimescaleDB must be reachable on localhost (or override the env vars below).
#
# Usage:
...
export KAFKA_GROUP_ID="${KAFKA_GROUP_ID:-rag-processor-group}"
export DATABASE_URL="${DATABASE_URL:-postgresql://txgen:txgen@localhost:5432/txgen}"
export LOG_LEVEL="${LOG_LEVEL:-INFO}"

# ── Helpers ────────────────────────────────────────────────────────────────────
GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; NC='\033[0m'
log()  { echo -e "${GREEN}[rag-processor]${NC} $*"; }
warn() { echo -e "${YELLOW}[rag-processor]${NC} $*"; }
err()  { echo -e "${RED}[rag-processor]${NC} $*" >&2; }
